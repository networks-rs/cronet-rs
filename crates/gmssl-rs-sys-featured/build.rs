...
fn main() {
    ...
    // Disable optional GmSSL features not selected by Cargo to keep builds
    // small. Environment overrides remain available for upstream parity.
    ...
    for feature in optional_features {
        ...
        let cargo_feature = match *feature {
            "ENABLE_AES" => env::var_os("CARGO_FEATURE_AES").is_some(),
            "ENABLE_SHA2" => env::var_os("CARGO_FEATURE_SHA2").is_some(),
            "ENABLE_TLS" => env::var_os("CARGO_FEATURE_TLS").is_some(),
            _ => false,
        };
        let environment_enabled =
            env::var(&env_var).is_ok_and(|value| matches!(value.as_str(), "ON" | "on" | "1"));
        if cargo_feature || environment_enabled {

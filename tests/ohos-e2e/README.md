...
not fix a CMake ABI or Native SDK API version.
...
The HAP exercises real local HTTP traffic through the safe binding: buffered
and `AsyncRead` uploads, streaming response reads, response size enforcement,
request metrics and finished events, cancellation, asynchronous engine
shutdown, and the bidirectional-stream failure path.
